...
@objc public protocol QuickTimePlayerDocument: SBObjectProtocol, QuickTimePlayerGenericMethods {
    ...
    @objc optional func exportIn(_ in_: URL!, usingSettingsPreset: String!) // Export a movie to another file
    ...
    @objc optional func setLooping(_ looping: Bool) // Is the movie playing in a loop?
    @objc optional func setMuted(_ muted: Bool) // Is the movie muted?
    @objc optional func setRate(_ rate: Double) // The current rate of the movie.
    @objc optional func setPresenting(_ presenting: Bool) // Is the movie presented in full screen?
